...
trait RegistrationWhitelist {
  ...
  lazy val config: AppConfig = FrontendAppConfig
  ...
  val returnDefaultCompRegProfile: (String) => CompanyRegistrationProfile =
    regId => CompanyRegistrationProfile(status = "accepted", transactionId = returnDefaultTransId(regId))
