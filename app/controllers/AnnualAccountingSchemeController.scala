...
class AnnualAccountingSchemeController @Inject()(appConfig: FrontendAppConfig,
                                         ...
                                         formProvider: AnnualAccountingSchemeFormProvider) extends FrontendController with I18nSupport {
  ...
  def onPageLoad() = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      Ok(annualAccountingScheme(appConfig, preparedForm, NormalMode))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(annualAccountingScheme(appConfig, formWithErrors, NormalMode))),
        (value) =>
