...
class CompletionCapacityFillingInForControllerSpec extends ControllerSpecBase {
  ...
  val form = formProvider(CompletionCapacityFillingInForId)(officersList)

  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new CompletionCapacityFillingInForController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute), FakeCacheIdentifierAction,
      dataRetrievalAction, new DataRequiredActionImpl, formProvider, mockIIService)

  def viewAsString(form: Form[_] = form) = completionCapacityFillingInFor(frontendAppConfig, form, NormalMode, CompletionCapacity.multipleOfficers(officersList, false))(fakeRequest, messages).toString

  "CompletionCapacityFillingInFor Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(CompletionCapacityFillingInForId.toString -> JsString(officersList.head.generateId))
