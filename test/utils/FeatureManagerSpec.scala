...
class FeatureManagerSpec extends PlaySpec with BeforeAndAfterEach {
  ...
  val vatFeatureSwitch      = new VATFeatureSwitchImpl(featureSwitch)
  ...
  "VATFeatureSwitch" should {
    "return a disabled feature when the associated system property doesn't exist" in {
      vatFeatureSwitch.useCrStubbed.enabled mustBe false
    }

    "return an enabled feature when the associated system property is true" in {
      featureSwitch.enable(vatFeatureSwitch.useCrStubbed)

      vatFeatureSwitch.useCrStubbed.enabled mustBe true
    }

    "return a disable feature when the associated system property is false" in {
      featureSwitch.disable(vatFeatureSwitch.useCrStubbed)
